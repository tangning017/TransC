...
void update(INT e1, INT e2, INT rel)
{
	...
	relationVecDao[lastar + ii] += g2 * entityVecDao[lasta2 + ii];
    }
...
void train_kb(INT e1_a, INT e2_a, INT rel_a, INT e1_b, INT e2_b, INT rel_b, INT flag, REAL* tmp)
{
    REAL e1, e2, e3, e4;
...
void* transDtrainMode(void* con)
{
	...
	if (randd(id) % 1000 < pr) {
	    ...
	    train_kb(trainList[i].h, trainList[i].t, trainList[i].r, trainList[i].h, j, trainList[i].r, -1, tmp);
	} else {
	    j = corrupt_tail(id, trainList[i].t, trainList[i].r);
	    train_kb(trainList[i].h, trainList[i].t, trainList[i].r, j, trainList[i].t, trainList[i].r, 1, tmp);
	}
...
void* train(void* con)
{
    ...
    nbatches = transDLen / transDBatch / threads;
	...
	printf("epoch %d lr %f res %f line %f\n", epoch, alpha, res, line_res);
...
void setparameters(int argc, char** argv)
{
    ...
    if ((i = ArgPos((char*)"-sizeR", argc, argv)) > 0)
	dimensionR = atoi(argv[i + 1]);
	...
	alpha = atof(argv[i + 1]);
    if ((i = ArgPos((char*)"-con_lambda", argc, argv)) > 0)
	con_lambda = atof(argv[i + 1]);

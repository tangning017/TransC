...
void update(INT e1, INT e2, INT rel, REAL tmp1, REAL tmp2)
{
    REAL x1 = 0, x2 = 0, g1, g2;
    INT lasta1 = e1 * dimension;
    INT lasta2 = e2 * dimension;
    INT lastar = rel * dimensionR;

    for (INT ii = 0; ii < dimension; ii++) {
	x1 += (entityVec[lasta1 + ii] - tmp1 * A[lastar + ii]) * relationVec[lastar + ii];
	x2 += (entityVec[lasta2 + ii] - tmp2 * A[lastar + ii]) * relationVec[lastar + ii];
    }
	...
	relationVecDao[lastar + ii] += g1 * (entityVec[lasta1 + ii] - tmp1 * A[lastar + ii]);
	A[lastar + ii] -= g1 * tmp1 * relationVec[lastar + ii];

	entityVecDao[lasta2 + ii] += g2 * relationVecDao[lastar + ii];
	relationVecDao[lastar + ii] += g2 * (entityVecDao[lasta2 + ii] - tmp2 * A[lastar + ii]);
	A[lastar + ii] -= g2 * tmp2 * relationVec[lastar + ii];
    }
...
void train_kb(INT e1_a, INT e2_a, INT rel_a, INT e1_b, INT e2_b, INT rel_b, REAL *p, REAL *n, int flag) {
	...
	if (sum1 + margin > sum2) {
		...
		if (flag == 1)
		    update(e1_a, e1_b, rel_a, tmp1, tmp2);
		else
		    update(e2_a, e2_b, rel_b, tmp3, tmp4);
	}
}
...
void* trainMode(void *con) {
	...
	for (INT k = Batch; k >= 0; k--) {
		...
		if (randd(id) % 1000 < pr) {
			...
			train_kb(trainList[i].h, trainList[i].t, trainList[i].r, trainList[i].h, j, trainList[i].r, positive, negative, -1);
		} else {
			j = corrupt_tail(id, trainList[i].t, trainList[i].r);
			train_kb(trainList[i].h, trainList[i].t, trainList[i].r, j, trainList[i].t, trainList[i].r, positive, negative, 1);
		}
...
void* train(void *con) {
	...
	for (INT epoch = 0; epoch < trainTimes; epoch++) {
		...
		printf("epoch %d lr %f res %f line_res %f\n", epoch, alpha, res, line_res);
		alpha = (1 - 1.0*epoch/trainTimes)*tmp;
	}
}
